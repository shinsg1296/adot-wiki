...
## 4-2. 에이닷 계정으로 우리 관제에 로그인하기 (SSO — 2026-08-19 조사)

원장 질문: *"VOC처럼 에이닷 로그인을 활용해 우리 사이트에 로그인할 수 없나?"* → **된다. 다만 본사 승인이 필요하다.**

VOC(`voc.dshare.co.kr`)가 쓰는 방식이 표준 SSO였고, CRM 로그인 페이지 **주석에 근거가 그대로 있다**:
> "외부 서비스가 `?redirect_uri=`로 진입시킨 경우… **서버가 화이트리스트 검증**(inc/login.php)"

```
우리 관제 로그인
 → crm.adotenglish.com/login.html?redirect_uri=<우리 콜백>
 → (에이닷 아이디로 로그인)
 → 우리 콜백?code=... 로 되돌아옴 → code를 사용자 정보로 교환 → 우리 세션 발급
```

VOC 쪽 엔드포인트(번들에서 확인): `/api/v1/auth/login` · **`/api/v1/auth/crm-callback`** · `/api/v1/auth/refresh`.
로그인 화면 문구도 "CRM 계정으로 로그인"이다. 현재 화이트리스트에 등록된 호스트는 `cms.adotenglish.com`,
`voc.dshare.co.kr` 계열뿐이라 **우리 도메인을 넣어 보내면 CRM이 거부한다.**

**본사에 요청할 것**
1. SSO 화이트리스트에 콜백 등록 — `https://hawecontrol.com/login/callback` (+ 지점별 서브도메인)
2. `code` 검증 엔드포인트 사양 (VOC의 `crm-callback`에 해당). **이건 코드로는 알 수 없고 본사에 물어야 한다.**

**하지 말 것 — 비밀번호 대신 받기.** 아이디·비번을 우리 서버가 받아 CRM에 대신 로그인하는 방식은 지금도
기술적으로 되지만(빠른채점이 이미 CRM에 로그인한다), **남의 본사 비밀번호가 우리 서버를 지나간다.**
본사 CRM은 OTP까지 쓰므로 더 나쁘다. 정식 경로가 이미 열려 있으니 우회로를 쓸 이유가 없다.

**상태: 보류**(원장 "이후에 하자" 2026-08-19). 협상 카드로도 쓸 만하다 — 본사 계정으로 로그인되는 학원
시스템은 본사 입장에서 이탈 방지 효과가 있어 요청을 들어줄 유인이 된다.
...
## 6. 본사에 등록해도 관제 명단은 자동으로 늘지 않는다 (2026-08-19)

원장 질문: *"본사 기준으로 재등록 처리되면 자동 반영되나?"* → **절반만.**

| 기능 | 본사에서 처리되면 | 이유 |
|---|---|---|
| 강의 수강 · 출결 · 본사 설정 대조 | **자동** | 카드를 열 때 CRM을 실시간 조회 |
| **관제 명단(학생 카드 자체)** | **수동** | 명단은 파일이고 갱신하는 자동 작업이 없다 |
| 단어앱 로그인 | **수동** | 로그인 명단이 스크립트 코드에 박혀 있다 |

즉 **신규·재등록 학생은 사람이 관제에 넣지 않으면 아예 없는 상태로 남는다**(실제 사례: 김가윤 2026-08-19).
본사 학생 목록은 **10분 캐시**라 방금 바꾼 값은 최대 10분 늦게 보인다(화면의 *새로 대조* 버튼으로 즉시 갱신).

**조치** — 관제가 *본사에는 있는데 관제에 없는 학생*을 알려주고 버튼으로 넣게 했다(원장 선택: 알림형).
완전 자동으로 하지 않은 이유는, 본사 데이터가 비어 있거나(담당T·요일이 비는 경우가 실제로 있다)
퇴원 처리와 얽히면 명단이 멋대로 바뀌기 때문이다. **사람이 한 번 보고 누르는 편이 안전하다.**
